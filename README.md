...
- **Captures every request/response** to domains you've explicitly opted in (see Target Scope
  below) as structured JSON, browsable and searchable from a dashboard tab inside Caido.
...
- **Exports a Markdown report** of everything captured — target scope, endpoints, skills map,
...
## Target Scope — opt-in only

The plugin does nothing until you explicitly add a domain to its Target Scope from the dashboard.
Traffic to any host not on that list is ignored entirely — it's not filtered out after capture,
it's never captured in the first place. This keeps the plugin from silently hoovering up every
domain your browser happens to talk to while Caido is running.

## Using it

1. Install the plugin (see Installation below) and open the **AI Recon Watcher** tab from the
   Caido sidebar.
2. Add the domain(s) you're actively testing to **Target Scope**.
3. Browse the target normally, or trigger requests through Caido's Proxy/Replay as usual. Traffic
   to scoped domains is captured and analyzed automatically.
4. Already have a request in Caido's history from before you added the scope? Right-click it (in
   the request list, the request editor, or a response) and choose **Send to AI Recon Watcher** —
   it adds the host to scope and analyzes that exchange immediately, so you don't have to wait for
   new traffic.
...
6. Use **Export Report** for a Markdown summary, or **Clear Data** to wipe captured
   captures/endpoints/skills (Target Scope itself is preserved across a clear — it's an engagement
   setting, not accumulated data).
...
- `packages/frontend` — dashboard UI: summary stats, Target Scope management, endpoints table,
  Skills/Tools map, recent captures log with search, and the right-click command/menu
  integrations.
